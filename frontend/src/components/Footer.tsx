...
const FooterSection: React.FC<FooterSectionProps> = React.memo(({ section }) => (
  <div className="space-y-3">
    <div className={cn(
      "flex items-center gap-2 p-2 -ml-2 rounded-lg group hover:bg-primary/5 transition-colors duration-300"
    )}>
      <section.icon className={cn(
        "h-4 w-4 text-primary/80 group-hover:text-primary transition-colors duration-300"
      )} />
      ...
      {section.links.map((link) => (
        <li key={link.label}>
          <Link
            to={link.href}
            className={cn(
              "group flex items-center gap-2 text-sm text-muted-foreground hover:text-foreground transition-colors duration-300"
            )}
            {...(link.isExternal ? { target: '_blank', rel: 'noopener noreferrer' } : {})}
          >
            {link.icon && (
              <link.icon className={cn(
                "h-3.5 w-3.5 text-muted-foreground/70 group-hover:text-primary/80 transition-colors duration-300"
              )} />
            )}
            <span className="relative">
              {link.label}
              <span className={cn(
                "absolute inset-x-0 -bottom-0.5 h-px bg-primary/50 scale-x-0 group-hover:scale-x-100 transition-transform duration-300 origin-left"
              )} />
            </span>
            {link.isExternal && (
              <ArrowUpRightIcon className={cn(
                "h-3 w-3 opacity-50 group-hover:opacity-100 transition-opacity duration-300"
              )} />
            )}
          </Link>
        </li>
...
const CertificationBadge: React.FC<CertificationBadgeProps> = React.memo(({ icon: Icon, label }) => (
  <div className={cn(
    "flex items-center gap-2 px-3 py-1.5 rounded-full bg-muted/50 hover:bg-muted/70 border border-primary/10 hover:border-primary/20 transition-all duration-300 group cursor-pointer"
  )}>
    <Icon className={cn(
      "h-4 w-4 text-primary/80 group-hover:text-primary transition-colors duration-300"
    )} />
...
const Footer: React.FC = React.memo(() => {
  ...
  return (
    <footer className={cn(
      "w-full border-t bg-muted/30 relative overflow-hidden"
    )}>
      ...
      <div className="container relative">
        ...
        <div className={cn(
          "py-6 border-t border-border/50 backdrop-blur-sm"
        )}>
              ...
              {['Privacy Policy', 'Terms of Service', 'Security'].map((item, index) => (
                <React.Fragment key={item}>
                  <Link
                    ...
                    className={cn(
                      "text-sm text-muted-foreground hover:text-foreground transition-colors duration-300"
                    )}
                  >
                    {item}
                  </Link>
          ...
          <div className="mt-6 text-center">
            <p className={cn(
              "text-sm bg-gradient-to-r from-primary/70 via-primary/50 to-primary/70 bg-clip-text hover:text-transparent transition-all duration-500"
            )}>
